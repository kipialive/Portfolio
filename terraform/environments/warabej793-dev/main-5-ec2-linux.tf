...
locals {
  ...
  backend_ec2_subdomain           = "dev.backend"
}
...
module "ec2-al2023" {
  ...
  # ami                    = "ami-093a4ad9a8cc370f4" # Exist only in Oregon (us-west-2)
...
module "dns_record_for_backend_ec2" {
  ...
  records = [
    {
      name = local.backend_ec2_subdomain
      type = "A"
      ttl     = 300
      records = [
        module.ec2-al2023.private_ip,
      ]
    },
  ]
