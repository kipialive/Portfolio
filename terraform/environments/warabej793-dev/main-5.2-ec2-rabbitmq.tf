...
locals {
  ec2_rabbitmq_name                           = "rabbitmq-${var.env_name_short}"
  ec2_rabbitmq_root_block_device_volume_size  = 20
  ec2_rabbitmq_instance_type                  = "t2.medium" # Change to "xxxxxxxx"

  ec2_rabbitmq_user_data              = <<-EOT
    #!/bin/bash
    echo "Hello Terraform!"
    APP_FOLDER=warabej793-dev
  ...
  rabbitmq_ec2_subdomain          = "dev.rabbitmq"
}
...
module "ec2-ubuntu-server-rabbitmq" {
  ...
  iam_instance_profile        = local.iam_role_name_al2023_instance

  user_data_base64            = base64encode(local.ec2_rabbitmq_user_data)
  ...
  root_block_device = [
    {
      volume_type           = "gp3"
      throughput            = 200
      volume_size           = local.ec2_rabbitmq_root_block_device_volume_size
      encrypted             = true
      delete_on_termination = true
      tags = {
        Name = "ubuntu-server-rabbitmq-root-block"
...
module "rabbitmq-security-group" {
  ...
  name        = "rabbitmq-${var.env_name_short}-sg"
  ...
  ingress_with_cidr_blocks = [
    ...
    },
    {
      ...
      to_port     = 5672
      protocol    = "tcp"
      description = "RabbitMQ Application"
      cidr_blocks = "${var.vpc_cidr}"
    },
    {
      from_port   = 15672
...
module "dns_record_for_rabbitmq_ec2" {
  ...
  records = [
    {
      name = local.rabbitmq_ec2_subdomain
      type = "A"
      ttl     = 300
      records = [
        module.ec2-ubuntu-server-rabbitmq.private_ip,
      ]
    },
  ]
